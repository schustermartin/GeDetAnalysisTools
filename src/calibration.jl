function determine_daq_core_calibration_constant(m::Measurement)
	daq_core_energies = GeDetMeasurements.get_daq_energies(m)[1, :]
	h = fit(Histogram, daq_core_energies, nbins=10000, closed=:left)
	c, pcg_hist = SpectrumAnalyser.determine_calibration_constant_through_peak_ratios(h)
	return c, pcg_hist
end

function determine_calibration_matrix_with_daq_energies(m::Measurement)
	@fastmath @inbounds begin 
		daq_core_energies = transpose(GeDetMeasurements.get_daq_energies(m))
		T = Float32
		...
		c0_daq::T, pcg_hist = SpectrumAnalyser.determine_calibration_constant_through_peak_ratios(h_core)
		...
		for iseg in 1:n_channel-1
	        ...
	        cal_peak_pos[iseg] = mp[cal_peak_idx]
		end
