...
function determine_superpulse(m::Measurement, channel::Int, energyrange::AbstractRange; T::Type=Float32)
	...
	try 
		...
		superpulses = zeros(T, n_samples, n_channel)
		...
		@showprogress for evt_range in evt_ranges
			...
			for i in 1:length(evt_range)
				...
				if elimit_max >= energies[core, ievt] >= elimit_min
					if elimit_max >= energies[channel, ievt] >= elimit_min
						...
						superpulses += GeDetPulseShapeAnalysisToolsTmp.calibrate_pulses(tdc_pulses, c)
					end
				end
			end
		end
