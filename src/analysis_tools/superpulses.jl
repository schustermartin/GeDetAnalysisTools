	...
	@showprogress for evt_range in evt_ranges
		...
		for i in 1:length(evt_range)
			...
			if ss_event(energies[1, ievt], energies[channel, ievt], elimit_min, elimit_max)
				...
				tdc_pulse = GeDetPulseShapeAnalysisToolsTmp.baseline_substraction_and_decay_correction(Float32.(chunk_pulses[:, :, i]), bl, bl_inv, decay_factors); 
				superpulse += GeDetPulseShapeAnalysisToolsTmp.calibrate_pulses(tdc_pulse, c)
			end
		end
	end
	close(h5f)
