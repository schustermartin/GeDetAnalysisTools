function determine_individual_decay_time_constants( m; take_every_n_sample_for_fit = 1 )
    println(m.name)
    T::DataType = Float32
    ...
    @info "Tau Decay Fitting (linear regression): $n_total_events events to process"
